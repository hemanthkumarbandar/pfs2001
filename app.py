    ...
    if session.get('user'):
        ...
        if request.method=="POST":
            try:
                ...
                desc=request.form["desc"]
                cursor=mydb.cursor(buffered=True)
                cursor.execute('update notes set title=%s,n_description=%s where n_id=%s',[title,desc,nid])
                mydb.commit()
                cursor.close()
            except Exception as e:
                print(e)
...
@app.route("/deletenotes/<nid>",methods=["POST","GET"])
def deletenotes(nid):
    if session.get('user'):
        try:
            cursor=mydb.cursor(buffered=True)
...
def getexceldata():
    if session.get('user'):
        try:
            ...
            #[['Notesid','Title','Content','Created_time'],[1, 'python','2024-12-16 11:14:25,],[2,'python','2024-12-16 11:14:53,]]
        ...
        try:
            if request.method=='POST':
                ...
                if(pattern.match(sdata)):
                    cursor=mydb.cursor(buffered=True)
                    cursor.execute('select * from notes where n_id like %s or title like %s or n_description like %s or create_at like %s',
                    s[data+'%',sdata+'%',sdata+'%',sdata+'%'])
